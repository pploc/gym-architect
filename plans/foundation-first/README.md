...
12. [Phase 11 — Implement `ms-gym-payment` and locked gate](11-payment-contracts.md)
...
14. Workout, Trainer, Promotion, Notification, and Analytics remain deferred. Payment work is limited to the membership-only G11 locked gate; no public Payment route or new wire contract is authorized.
...
| G11 — Payment implementation and locked gate | Membership-only SePay service, webhook, outbox, infrastructure, and unchanged Member event boundary pass | Zero-wire Buf checks; service/schema/webhook/outbox/security tests; real Payment-to-Member replay/overpay E2E; detached locked sources; protected CI; sanitized evidence; owner acceptance |
