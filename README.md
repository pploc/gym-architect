...
5. [`ms-gym-payment`](services/03-ms-gym-payment.md) — G11 complete: membership-only SePay `InitiatePayment`, verified webhook, and transactional outbox produce `payment.completed.v1`. G8 fake-payment remains a historical compatibility fixture.
...
- [Phase 11 — Implement `ms-gym-payment` and locked gate](plans/foundation-first/11-payment-contracts.md) — complete; locked evidence and user-directed accountable owner acceptance are recorded.
...
| Membership payment intents, SePay webhooks, and completed event | Payment (G11 complete) |
